...
- **速度冲刺（Speed Surge）**：在轨道上拾取“靴子”充能，点击后获得 3 秒 ×2 速度的全局加速，带有冷却与充能数上限。
- **风暴按钮（Egg Storm）**：消耗已有风暴次数并进入布置模式。

### 身份技能（Passive Skills（右下角）
- **幸运蛋（Lucky Egg）**：每第 5 个蛋升级为 +10 分超级蛋。
- **金币增益（Coin Rain）**：自动 +5 金币/秒，加快购买节奏。
- **急速孵化（Rapid Nest）**：每 0.5 秒额外生成 1 枚鸡蛋，提升单位时间流量。
...
6. 运行 Gradio Web Demo：`python app.py`
7. 浏览器访问控制台输出的地址（默认 http://127.0.0.1:7860），即可看到实时画面。
	- 左键点击游戏画面即可购买/放置道具；
